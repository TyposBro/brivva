...
import { type Env, toUserInfo } from "../core/types";
...
app.post("/test/reset-dev-user", async (c) => {
	...
	const DEV_USER_ID = "dev-user";
	// Delete children before parents to satisfy FK references.
	await c.env.DB.prepare(
		"DELETE FROM session_log_events WHERE session_id IN (SELECT id FROM sessions WHERE user_id = ?)",
	)
		.bind(DEV_USER_ID)
		.run();
	await c.env.DB.prepare(
