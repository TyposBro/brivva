...
pub(super) async fn forward_audio_to_gladia(
    audio_rx: Arc<tokio::sync::Mutex<tokio::sync::mpsc::UnboundedReceiver<Vec<u8>>>>,
    sink: Arc<tokio::sync::Mutex<
        futures_util::stream::SplitSink<WsStream, tungstenite::Message>,
    >>,
    accumulator: Arc<std::sync::Mutex<Vec<Vec<u8>>>>,
    session_id: String,
) {
    ...
    while let Some(data) = rx.recv().await {
        ...
        log_progress(&session_id, chunk_count, total_bytes);
        accumulate_audio(&accumulator, &data);
        if send_to_gladia(&sink, data, &session_id).await.is_err() { break; }
    }

    send_stop_recording(&sink, &session_id, chunk_count, total_bytes).await;
}
...
async fn send_stop_recording(
    ...
    use futures_util::SinkExt;
    info!("[STT:{}] sending stop_recording (total: {} chunks, {}KB)", session_id, chunk_count, total_bytes / 1024);
    let mut sink = sink.lock().await;
