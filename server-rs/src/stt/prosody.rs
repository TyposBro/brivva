...
fn compute_pitch_stats(samples: &[f32], sample_rate: u32) -> (f32, f32) {
    ...
    while start + frame_len <= samples.len() {
        if let Some(pitch) = estimate_frame_pitch(&samples[start..start + frame_len], min_lag, max_lag, sample_rate) {
            pitches.push(pitch);
        }
        start += hop_len;
    }

    let mean = if pitches.is_empty() { 0.0 } else { pitches.iter().sum::<f32>() / pitches.len() as f32 };
    let std = if pitches.is_empty() { 0.0 } else {
        (pitches.iter().map(|p| (p - mean).powi(2)).sum::<f32>() / pitches.len() as f32).sqrt()
    };
    (mean, std)
}
...
fn estimate_frame_pitch(frame: &[f32], min_lag: usize, max_lag: usize, sample_rate: u32) -> Option<f32> {
    ...
    if energy > 0.0 && best_corr / energy > AUTOCORRELATION_THRESHOLD {
        let pitch_hz = sample_rate as f32 / best_lag as f32;
        if pitch_hz > PITCH_MIN_HZ && pitch_hz < PITCH_MAX_HZ {
            return Some(pitch_hz);
        }
    }
    None
}
...
fn compute_pause_density(samples: &[f32], sample_rate: u32) -> f32 {
    ...
    let threshold = median * PAUSE_ENERGY_FRACTION;
