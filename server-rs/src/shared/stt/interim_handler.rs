...
fn send_interim_to_host(ctx: &SttContext, transcript: &str) {
    ...
    if let Some(session) = ctx.sessions.get(&ctx.session_id) {
        session.send_to_host(crate::features::broadcast::data::pipeline_helpers::to_ws(&ServerMsg::Interim {
...
struct DetectedChunk {
    boundary: crate::shared::stt::ChunkBoundary,
...
fn spawn_pipeline_if_needed(state: &mut SttState, ctx: &SttContext) {
    ...
    let req = crate::features::broadcast::data::pipeline_chunk::ChunkPipelineRequest {
        sessions: ctx.sessions.clone(),
        session_id: ctx.session_id.clone(),
        utterance_id: state.utterance_counter,
        source_lang: ctx.source_lang.clone(),
        style_params: StyleParams::default(),
        translate_api_key: ctx.translate_api_key.clone(),
        tts_api_key: ctx.tts_api_key.clone(),
        default_voice: ctx.default_voice.clone(),
        http_client: ctx.http_client.clone(),
    };
    state.chunk_pipeline_tx = crate::features::broadcast::data::pipeline_chunk::spawn_chunked_pipeline(req);
}
