...
async fn send_tts_request(
    ...
    let resp = post_request(client, &url, &body, req.api_key).await?;
...
async fn post_request(client: &reqwest::Client, url: &str, body: &serde_json::Value, api_key: &str) -> Result<reqwest::Response, String> {
    client
        .post(url)
...
async fn decode_and_truncate(mp3: &[u8], max_bytes: usize) -> Result<Vec<u8>, String> {
    let mut pcm = crate::features::broadcast::data::streaming::decode_mp3_to_pcm(mp3).await?;
...
fn append_to_stream(pcm: &[u8], streaming: Option<&crate::features::broadcast::data::streaming::StreamingPcm>) {
