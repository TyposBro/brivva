  ...
  return (
    <div className="bg-surface-container-low rounded-xl p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        {voiceReady && (
          <span className="text-xs text-secondary font-medium px-2 py-0.5 bg-secondary-container rounded-full">Active</span>
        )}
      </div>

      {isCloning ? <CloningProgress progress={cloneProgress} /> : <CloningPrompt voiceReady={voiceReady} onClone={onClone} />}
    </div>
  );
}
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-3">
        <div className="flex-1 h-2 bg-surface-container-high rounded-full overflow-hidden">
          <div className="h-full bg-secondary rounded-full transition-all duration-200" style={{ width: `${progress * 100}%` }} />
        </div>
      ...
      {progress < 1 && <p className="text-xs text-outline">Read the text above naturally into your microphone.</p>}
    </div>
  );
}

function CloningPrompt({ voiceReady, onClone }: { voiceReady: boolean; onClone: () => void }) {
  return (
    <div className="space-y-2">
      ...
      <button onClick={onClone} className="px-4 py-2 rounded-lg bg-secondary-container text-on-secondary-container text-sm font-semibold hover:opacity-90 transition-opacity">
