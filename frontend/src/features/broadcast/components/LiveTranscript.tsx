...
export function LiveTranscript({ transcripts, interim }: Props) {
  ...
  return (
    <div ref={scrollRef} className="bg-surface-container-lowest rounded-xl p-4 space-y-3 max-h-96 overflow-y-auto">
      ...
      {transcripts.length === 0 && !interim && (
        <div className="text-sm text-outline text-center py-4">Listening... speak into your microphone</div>
      )}
    </div>
  );
}
