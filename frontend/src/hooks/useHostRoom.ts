...
export function useHostRoom() {
  ...
  const handleMessage = useCallback((msg: RoomMessage) => {
    switch (msg.type) {
      ...
      case "final": {
        ...
        setUtterances((prev) => [...prev, { id: msg.utteranceId as number, transcript: text }]);
        setLiveTranscript("");
  ...
  const startRecording = useCallback(async () => {
    ...
    const analyserNode = await audio.current.start((buf) => socket.current.sendAudio(buf));
