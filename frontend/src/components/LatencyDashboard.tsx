...
const GAP_CRITICAL_THRESHOLD = 5;
...
function buildSegments(t: UtteranceTiming, maxMs: number) {
  return [
...
function TimingBar({ timing: t, maxMs, targetPct }: {
  timing: UtteranceTiming;
  maxMs: number;
  targetPct: number;
}) {
...
function TimingDetails({ timing: t }: { timing: UtteranceTiming }) {
  return (
