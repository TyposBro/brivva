...
| RS-015 | open | Grip integration is not proven by current YouTube-only stress runs. | 2026-05-03 `backlog_catchup_many_outputs` log contained only YouTube outputs; Grip was not started. Code now forces Grip output to `720x1280`, H.264, 1s keyframes, and <3 Mbps. | Run `grip_smoke` against fresh one-shot Grip creds and verify Grip Studio receives video/audio. | [Scenarios: Grip Smoke](scenarios.md#grip-smoke) |
