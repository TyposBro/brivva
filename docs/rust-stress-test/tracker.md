...
| RS-006 | patched | Provider failures must not charge customers for unusable vendor/platform time. | Soniox/ElevenLabs/RTMP provider health now persists into Workers failure windows; usage/summary subtract unbillable lang/source windows from billable minutes and expose unbillable audit buckets. Workers API tests pass, including ElevenLabs JA unbillable-window subtraction. ElevenLabs has a circuit breaker. | Validate bad Soniox key, bad ElevenLabs key/429, and platform-specific visible-live checks in e2e. Bad RTMP isolation is now covered by RS-014 local stress. | [Provider Failure And Billing](provider_failure_billing.md) |
...
| RS-014 | patched | One FFmpeg encode feeding multiple RTMP publishers can fail differently by platform/library. | Runner now has `one_bad_destination`: all configured good outputs plus one dead RTMP URL. 2026-05-05 local stress passed: bad RTMP tee slave failure observed, sibling YouTube live health observed, and media-drop/slow-encode assertions stayed clean. | Run scenario with production-like multi-output secrets; keep per-process publish fallback and platform soak. | [Scenarios: One Bad Destination](scenarios.md#one-bad-destination) |
...
| `dca2019` | patched | Added optional host-provided product/brand/offer translation terms from setup UI through Workers into Soniox context. | Validate with real product names/promo codes during next rehearsal. |
| `a7a9362` | patched | Excluded persisted provider failure windows from billable usage/summary and exposed unbillable audit buckets. | Run bad Soniox/bad ElevenLabs e2e; keep billing unit tests green. |
| `2bc53ff` | patched | Added one-bad-destination stress automation with output-health assertions. | Run with production-like multi-output secrets before paid events. |
