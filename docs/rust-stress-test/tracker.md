...
| RS-023 | patched | Translated tokens have no timestamps, so source timing must use source/original tokens while TTS budget can use available silence. | Soniox `start_ms`/`end_ms` parsing, `SourceUtteranceTiming`, and env-gated lookahead budget are patched. TTS logs source speech duration separately from available window and uses `tts_budget_ms()` for concision. | Live-validate `BRIVVA_TTS_LOOKAHEAD_BUDGET=1`: fast utterance + silence should show `available_window_method=NextUtteranceStart`; fallback should show `HoldTimeoutFallback` or `ResponseWallClock` explicitly. | [TTS Concision: Soniox Findings](tts_concision.md#soniox-findings) |
