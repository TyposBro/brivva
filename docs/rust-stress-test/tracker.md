...
| RS-015 | open | Grip integration is not proven by current YouTube-only stress runs. | Grip manual RTMP exists in product UI; smoke runner now supports `GRIP_RTMP_URL` + `STREAM_KEY_GRIP`. | Run `grip_smoke` against fresh one-shot Grip creds and verify Grip Studio receives video/audio. | [Scenarios: Grip Smoke](scenarios.md#grip-smoke) |
