...
| --- | --- | --- | --- | --- | --- |
| RS-001 | open | JA translated TTS can exceed live window and drop whole segments. | Last full e2e before stronger concision had 25 `tts segment queue overflow`, all `lang=ja`. | Add Soniox translate context instructions; rerun `backlog_catchup_many_outputs` and compare overflow/hard recovery counts. | [TTS Concision](tts_concision.md), [Live Failure Policy: Translated Audio Too Long](live_failure_policy.md#translated-audio-too-long) |
| RS-002 | open | Current concision fallback is deterministic and hand-authored; it should remain safety net, not main quality layer. | Code now removes filler and preserves commerce clauses, but phrase lists are not infinitely scalable. | Move concision upstream via Soniox `context.general.instructions`; later add feature-flag rewrite provider only if needed. | [TTS Concision: Better Than Hand Rules](tts_concision.md#better-than-hand-rules) |
...
| RS-031 | open | Full stress suite may not assert pass/fail on every bad signal yet. | Runner stores logs; humans still inspect several signals manually. | Add post-run log assertions for drops, speed, overflows, and hard recovery thresholds. | [Signals: Useful Greps](signals.md#useful-greps) |
