...
| RS-006 | patched | Provider failures must not charge customers for unusable vendor/platform time. | Soniox/ElevenLabs/RTMP provider health now persists into Workers failure windows; usage/summary subtract unbillable lang/source windows from billable minutes and expose unbillable audit buckets. ElevenLabs has a circuit breaker. | Validate bad Soniox key, bad ElevenLabs key/429, and bad RTMP destination in e2e; add platform-specific visible-live checks later. | [Provider Failure And Billing](provider_failure_billing.md) |
...
| RS-014 | patched | One FFmpeg encode feeding multiple RTMP publishers can fail differently by platform/library. | Runner now has `one_bad_destination`: all configured good outputs plus one dead RTMP URL, with output-health assertions that the bad legacy output fails while a YouTube sibling reaches live. | Run scenario with production-like multi-output secrets; keep per-process publish fallback and platform soak. | [Scenarios: One Bad Destination](scenarios.md#one-bad-destination) |
